...
export class DatosPersona implements OnChanges {
    ...
    if ('initialModel' in changes) {
      ...
      this.optionsApi.getOptions().subscribe({
        next: (options) => {
          ...
          // El catálogo de países llega de forma asíncrona: si el tipo de
          // documento auto-España ya estaba seleccionado (p.ej. viene de un
          // cliente encontrado) y todavía no teníamos país, lo completamos
          // ahora que ya tenemos el catálogo cargado.
          if (
  ...
  protected handleDocumentTypeChange(event: Event): void {
    ...
    if (documentType && AUTO_SPAIN_DOCUMENT_TYPES.includes(documentType)) {
      // DNI/NIF/CIF -> país España automático (criterio de aceptación).
      this.updateModel({ documentType, nationality: this.findSpainOptionValue() });
    } else {
      // NIE/NRT/pasaporte -> el país se limpia y hay que seleccionarlo a mano.
  ...
  /** Busca en el catálogo de países (ya cargado desde /catalogo/paises) la opción "España". */
  private findSpainOptionValue(): string | undefined {
    return this.options.nationalities.find(
      (option) => option.label.trim().toLowerCase() === SPAIN_LABEL,
    )?.value;
  }
}
