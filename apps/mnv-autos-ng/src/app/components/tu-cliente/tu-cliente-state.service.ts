...
export class TuClienteStateService {
  ...
  // Convenience flags for templates and logic
  readonly clientFound = computed(() => Boolean(this.clientResult()));
  readonly compactMode = computed(() => this.clientNotFound());
  ...
  setNewClientDocument(documentType: DatosPersonaModel['documentType'], documentNumber: string): void {
    this._state.update((current: TuClienteGlobalState) => ({
      ...
      datosPersona: {
        ...
        documentType,
        documentNumber,
        nationality: documentType === 'nif' || documentType === 'cif' ? 'ES' : undefined,
      },
    }));
  }
  ...
  private fillDatosPersonaFromClient(client: ClienteBusquedaResult): void {
    ...
    this._state.update((current: TuClienteGlobalState) => ({
      ...current,
      datosPersona: {
        documentType: (client.documentType.toUpperCase() === 'DNI' ? 'nif' : client.documentType.toLowerCase()) as DatosPersonaModel['documentType'],
