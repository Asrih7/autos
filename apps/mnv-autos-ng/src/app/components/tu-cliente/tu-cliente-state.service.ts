...
export class TuClienteStateService {
  ...
  readonly clientNotFound = computed(() => this._state().clientNotFound);

  readonly clientFound = computed(() => Boolean(this.clientResult()));
  readonly compactMode = computed(
    () => this.clientNotFound() || (!this.clientResult() && Boolean(this.operationType())),
  );

  readonly canContinue = computed(() => {
    const state = this._state();
    if (state.clientResult && state.operationType && !state.editingClient) {
      return true;
    }
    ...
    if (
      state.clientResult &&
      !state.editingClient &&
      !state.showPersonalDataStep &&
      !state.showAddressStep &&
      !state.showBirthdateStep
    ) {
      return true;
    }
    ...
    const hasPersonalData = state.clientResult
  ...
  setClientNotFound(value: boolean): void {
    this._state.update((current: TuClienteGlobalState) => ({
      ...current,
      clientNotFound: value,
    }));
  }

  startOperacionFromNotFound(type: OperacionTipo): boolean {
    this._state.update((current: TuClienteGlobalState) => ({
      ...current,
      clientNotFound: false,
      operationType: type,
  ...
  setNewClientDocument(documentType: DatosPersonaModel['documentType'], documentNumber: string): void {
    this._state.update((current: TuClienteGlobalState) => ({
      ...current,
      datosPersona: {
        ...this.emptyDatosPersona(),
        documentType,
        documentNumber,
        nationality: ['dni', 'nif', 'cif'].includes(documentType ?? '') ? 'ES' : undefined,
      },
    }));
  }
  ...
  startOperacion(type: OperacionTipo): boolean {
    ...
    this._state.update((current: TuClienteGlobalState) => ({
      ...
      showAddressStep:
        !clientResult && ['nif', 'cif'].includes(this.datosPersona().documentType ?? ''),
      showBirthdateStep: false,
    }));
    ...
    if (state.editingClient && state.clientResult) {
      this._state.update((current: TuClienteGlobalState) => ({
        ...
        clientResult: {
          ...
          address: current.direccion ? this.formatAddress(current.direccion) : current.clientResult?.address ?? '',
          birthDate: current.birthDate ?? current.clientResult?.birthDate,
        },
        editingClient: false,
        showPersonalDataStep: false,
        showAddressStep: false,
  ...
  setAddressError(value: string | null): void {
    this._state.update((current: TuClienteGlobalState) => ({ ...current, addressError: value }));
  }
  ...
  private fillDatosPersonaFromClient(client: ClienteBusquedaResult): void {
    ...
    const rawType = String(client.documentType ?? '').trim().toLowerCase();
    let normalizedType: DatosPersonaModel['documentType'] | undefined = undefined;
    if (rawType === 'dni') normalizedType = 'nif';
    else if (rawType === 'pas' || rawType === 'passport') normalizedType = 'passport';
    else if (['nif', 'cif', 'nie', 'nrt'].includes(rawType)) normalizedType = rawType as DatosPersonaModel['documentType'];
    ...
    this._state.update((current: TuClienteGlobalState) => ({
      ...
      datosPersona: {
        ...
        nationality: client.nationality === 'Española' ? 'ES' : client.nationality || undefined,
        firstName,
        firstSurname,
        secondSurname,
        businessName,
  ...
  private formatAddress(direccion: DatosDomicilioModel): string {
    return [
      direccion.tipoVia,
      direccion.nombreVia,
      direccion.numero,
